...
// [PUT] /api/sources/:id - Belirli bir source'u güncelle
app.put('/api/sources/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      // 1. Adım: Güncellenmek istenen kaydın bu kullanıcıya ait olup olmadığını kontrol et
      ...
      if (ownershipCheck.recordset.length === 0) {
        await transaction.rollback(); // İşlemi geri al
        return res.status(404).json({ message: 'Kaynak bulunamadı veya bu kaynağı düzenleme yetkiniz yok.' });
      }

      // 2. Adım: Ana SOURCE tablosunu güncelle
      await transaction.request()
        .input('sourceId', sql.Int, sourceId)
        .input('name', sql.NVarChar, name)
        .input('type', sql.NVarChar, type)
        .query('UPDATE [mosuser].[SOURCE] SET NAME = @name, TYPE = @type WHERE ID = @sourceId');

      // 3. Adım: Eski detayları sil (en temiz güncelleme yöntemi)
      await transaction.request()
        .input('sourceId', sql.Int, sourceId)
        .query('DELETE FROM [mosuser].[SOURCE_INFO] WHERE SOURCE_ID = @sourceId');

      // 4. Adım: Yeni detayları ekle
      for (const [property, value] of Object.entries(details)) {
        await transaction.request()
          .input('sourceId', sql.Int, sourceId)
          .input('property', sql.NVarChar, property)
      ...
      // 5. Adım: Her şey yolundaysa işlemi onayla
      await transaction.commit();
      res.status(200).json({ message: 'Source başarıyla güncellendi.' });

    } catch (innerErr) {
      // Transaction içinde bir hata olursa tüm işlemleri geri al
      await transaction.rollback();
      console.error('Source güncelleme transaction hatası:', innerErr);
      res.status(500).json({ message: 'Güncelleme sırasında bir veritabanı hatası oluştu.' });
    }
  } catch (err) {
    console.error('Source güncelleme genel hatası:', err);
