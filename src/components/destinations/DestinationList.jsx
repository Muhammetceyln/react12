...
const DestinationList = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 204) {
        ...
        let msg = `Delete error: ${res.status}`;
        try {
        ...
        console.error('Details of delete error:', msg);
        setSnackbar({ open: true, message: msg, severity: 'error' });
      }
    } catch (err) {
      console.error('Silme isteği hatası:', err);
      setSnackbar({ open: true, message: 'There is a error during deletion request', severity: 'error' });
    } finally {
      // Temizlik
  ...
  const handleEdit = async (destination) => {
    // Popup'ı EDIT modunda açacağımızı belirtiyoruz
    setFromDetails(false); // <--- DEĞİŞİKLİK BURADA
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Dialog open={confirmOpen} onClose={cancelDelete}>
        ...
        <DialogActions>
          ...
          <Button onClick={confirmDelete} variant="contained" disabled={deletingLoading}>
            {deletingLoading ? <CircularProgress size={18} /> : "Delete"}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Snackbar
        ...
        anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }} // (Opsiyonel) Konumlandırma
      >
        {/* İçine Alert bileşenini ekliyoruz */}
        <Alert
          onClose={() => setSnackbar(s => ({ ...s, open: false }))}
          severity={snackbar.severity} // <-- Başarı/Hata rengini buradan alır
          sx={{ width: '100%' }}
        >
          {snackbar.message}
        </Alert>
