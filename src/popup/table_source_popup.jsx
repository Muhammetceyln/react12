...
import { Cloud, Storage, Business } from '@mui/icons-material';
...
// table_source_popup.jsx içindeysen (src/popup/)
...
export default function TableSourcePopup({ open, setOpen, onSave, initialData = null }) {
    ...
    <Box sx={{ mb: 3 }}>
      <FormControl fullWidth>
        <InputLabel className="fontSizelbl">Database Connection</InputLabel>
        <Select
        ...
        >
          {loading ? (
            ...
            connections.map((conn) => {
              let imagePath;
              // Bağlantı türüne göre resim seçimi
              switch (conn.type) {
                case 'HANA':
                  imagePath = sapHana; // HANA resmi
                  break;
                case 'MSSQL':
                  imagePath = mssqlLogo; // MSSQL resmi
                  break;
                case 'SAP':
                  imagePath = sapIcon; // SAP resmi
                  break;
                default:
                  imagePath = null;
                  break;
                ...
                <MenuItem key={conn.id} value={conn.id}>
                  ...
                  {conn.name} ({conn.type})
                    ...
                    {selectedSchema && (
                        <Box sx={{ mb: 3 }}>
                            <FormControl fullWidth>
                                <InputLabel className="fontSizelbl">Table</InputLabel>
                                <Select
                                    value={selectedTable}
                                    label="Table"
      ...
      />
                    ...
                    {showPreview && previewData.length > 0 && (
                        <Accordion sx={{ mt: 3 }}>
                            ...
                            <AccordionDetails>
<Paper className="custom-table" sx={{ overflow: 'auto', maxHeight: 400 }}>
...
<DialogActions>
  <Button onClick={handleCancel} className="small-btn">
    Cancel
  </Button>
  <Button
    onClick={handleSave}
